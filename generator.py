...
from syntax import parser, NonterminalNode, TerminalNode
from lexical import lexer
from c_std import c_stds
from methods import format_python_code, preprocess, indent
...
class Generator:
    ...
    def translate(self, input_file_name, output_file_name):
        try:
            # 预处理
            success, file_content = preprocess(input_file_name)
            if not success:
                print(file_content)
                return
            # 词法、语法分析生成语法树
            tree = parser.parse(file_content,lexer=lexer)
            # 语义处理
            raw_outcome = self.process(tree)

            # 转化为正确格式，优化代码风格，加上首尾代码
            out = indent(raw_outcome)
            #out = format_python_code(self.head + out + self.tail)
    ...
    def flag_calculate(self, tree, flag_list):
        if tree.type == 'struct_or_union_specifier':
    ...
    def is_function_declaration(self, tree):
        for child in tree.children:
            ...
            if child.type == 'direct_declarator':
                if len(child.children) > 1 and isinstance(child.children[1], TerminalNode) \
    ...
    def name_replacement(self, tree, is_declarator=False):
        if isinstance(tree, NonterminalNode):
            ...
            # 声明语句中的表达式部分不算声明
            elif tree.type == 'primary_expression':
                for child in tree.children:
                    self.name_replacement(child, False)
            # 结构体内部变量直接跳过
            elif tree.type == 'struct_or_union_specifier':
                return
                # for child in tree.children:
                #     self.name_replacement(child, False)
            #
            elif tree.type == 'postfix_expression' and len(tree.children) == 3 \
                    and isinstance(tree.children[2], TerminalNode) and tree.children[2].type == 'IDENTIFIER':
                for child in tree.children[:2]:
                    self.name_replacement(child, False)
            # 选择或循环语句，进入新一层作用域
            elif tree.type == 'iteration_statement' or tree.type == 'selection_statement':
                # 进入作用域，保存副本
                table_copy = copy.deepcopy(self.variable_table)
                for child in tree.children:
    ...
    def code_compose(self, tree, code_list, flag):
        # 前置 ++/--
        if tree.type == 'unary_expression' and isinstance(tree.children[0], TerminalNode):
        ...
        # 后置 ++/--
        elif tree.type == 'postfix_expression' and len(tree.children) == 2:
        ...
        # return 语句
        elif tree.type == 'jump_statement' and tree.children[0].type == 'return':
            if len(tree.children) == 2:
                """
                return
                """
        ...
        # 选择语句
        elif tree.type == 'selection_statement':
        ...
        # 循环语句
        elif tree.type == 'iteration_statement':
            #  while {
            #      ...
            #  }
        ...
        # 语句块的换行
        elif tree.type == 'block_item_list':
            lst = []
        ...
        # {}作用域 去除{}
        elif tree.type == 'compound_statement':
        ...
        # 函数声明, 返回值类型变为def,
        elif tree.type == 'function_definition':
        ...
        # 函数参数列表中参数类型的去除
        elif tree.type == 'parameter_declaration' and len(tree.children) == 2:
            """
            变量（没有类型）
            """
            return code_list[1]
        ...
        # 函数列表中数组“[]”的去除
        elif tree.type == 'direct_declarator' and len(tree.children) == 3 and tree.children[1].value == '[':
            """
            数组变量名
            """
        ...
        elif tree.type == 'init_declarator_list':
        ...
        elif tree.type == 'struct_or_union_specifier' and len(tree.children) == 2:
            """
            结构体名称
            """
            return code_list[1]
        ...
        elif tree.type == 'struct_or_union_specifier' and len(tree.children) == 5:
            """
            class 结构体名称:
                成员变量（缩进+1）
            """
        ...
        # 结构体定义内的语句列表
        elif tree.type == 'struct_declaration_list':
            lst = []
            for code in code_list:
                for c in code:
                    lst.append(c)
        ...
        # 声明语句
        if tree.type == 'declaration' or tree.type == 'struct_declaration':
        ...
        elif tree.type == 'direct_declarator' and len(tree.children) == 4 and \
                isinstance(tree.children[2], NonterminalNode) and \
                tree.children[2].type == 'assignment_expression':
            """
            数组名 = [None] * 长度
            """
            return [code_list[0][0] + '=[' + 'None' + ']*' + code_list[2][0]]

        elif tree.type == 'init_declarator' and len(tree.children) == 3 and code_list[0][0].find('[') >= 0:
        ...
        # 去除数组的[]
        elif tree.type == 'initializer' and len(tree.children) == 3:
            """
            []中的值
            """
            return code_list[1]
